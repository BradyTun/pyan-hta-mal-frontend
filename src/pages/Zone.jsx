...
import { FaExclamationTriangle, FaCheckCircle, FaMapMarkerAlt } from "react-icons/fa";
...
const Zone = () => {
    const zone = {
        ...
        riskLevel: 8,
        topNeeds: [
            { name: "Water", level: "Critical", note: "Shortage of clean drinking water" },
            { name: "Food", level: "High", note: "Limited food supplies available" },
            { name: "Shelter", level: "Moderate", note: "Temporary shelters are overcrowded" },
        ],
        description: "Zone A is heavily affected by recent floods, requiring immediate attention.",
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-secondary p-6">
            ...
            <div className="max-w-3xl mx-auto mt-16 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg">
                {/* Last Updated */}
                <p className="text-sm text-gray-500 dark:text-gray-400">Last Updated: {zone.lastUpdated}</p>

                {/* Description */}
                <p className="text-lg text-gray-700 dark:text-gray-300 mt-2">{zone.description}</p>
                ...
                <div className="mt-6">
                    ...
                    <span
                        className={`inline-block px-4 py-2 rounded-lg font-medium mt-2 ${getRiskLevelColor(zone.riskLevel)}`}
                    >
                        {zone.riskLevel} / 10
                    </span>
                </div>
                    ...
                    <ul className="mt-2 space-y-3">
                        {zone.topNeeds.map((need, index) => (
                            <li key={index} className="flex items-center gap-3 bg-gray-100 dark:bg-gray-700 p-3 rounded-lg">
                                ...
                                <div>
                                    <span className="font-semibold text-gray-800 dark:text-gray-200">{need.name}:</span>
                                    <span className="text-primary ml-1">{need.level}</span>
                                    <p className="text-sm text-gray-600 dark:text-gray-400">{need.note}</p>
                                </div>
                            </li>
                        ))}
                    </ul>
                        ...
                        {zone.risks.map((risk, index) => (
                            <li key={index} className="flex items-center gap-3 bg-gray-100 dark:bg-gray-700 p-3 rounded-lg">
                                ...
                                <span className="text-gray-800 dark:text-gray-200 font-medium">
                                    {risk.name}{" "}
                                    <span className="text-primary">(Severity: {risk.severity})</span>
                                </span>
                            </li>
                        ))}
                    </ul>
